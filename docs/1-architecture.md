...
| Web 面板 | Nuxt.js (静态构建) | 预编译，无需 Node.js 运行时 |
...
│   └── ui -> ../dashboard/public # Dashboard 软链接
│
├── volumes/                      # 用户数据目录
│   ├── geoip/                    # GeoIP 数据库
│   │   └── Country.mmdb
...
│  │  External API (9090) │ Dashboard UI                      │     │
│  └────────────────────────────────────────────────────────┘     │
└─────────────────────────────────────────────────────────────────┘
```
...
| **13** | 结果输出 | 显示 Dashboard、Secret、下一步操作 |
...
Mihomo 内核有安全路径限制，`external-ui` 必须在允许的目录下：

```bash
# 默认允许的路径
SAFE_PATHS=/opt/clash-for-linux:/root/.config/mihomo
```

如果遇到 `path is not subpath of home directory or SAFE_PATHS` 错误，需要：

1. 设置 `SAFE_PATHS` 环境变量包含项目目录
2. 或将 UI 文件放到 `$HOME/.config/mihomo` 目录
```
